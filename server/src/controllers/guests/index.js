...
routes.push({
    ...
    middleware: (req, res, next) => {
        ...
        sequelize.query("SELECT p.create,p.read,p.edit,p.delete,p.module from role_permissions as rp,permissions as p where p.module='guest' and rp.pid = p.id and rp.rid = (SELECT rid FROM user_role WHERE uid =?)",
            {
...
routes.push({
    ...
    middleware: (req, res, next) => {
        ...
        sequelize.query("select count(*) as total from guests")
        .then((result) => {
                res.send(200, result);
                return next();
            }).catch((err) => {
