        ...
        if (form.nights > 0) {
            models.rooms.findAll({
                ...
                if (data.length > 0) {
                    var amount = 0;
                    if (data.length == 1) {
                        amount = data[0].dataValues.price_night;
                    } else {
            ...
            models.rooms.findAll({
                ...
                if (data.length > 0) {
                    ...
                    return models.sequelize.transaction((t) => {
                        ...
                        return models.bookings.findOne({
                            where: {
                                id: {
                                    [Sequelize.Op.eq]: req.params.id
                                }
...
routes.push({
    meta: {
        ...
        method: 'GET',
        paths: [
            '/bookingPermission/:id'
        ]
    },
    middleware: (req, res, next) => {

        sequelize.query("SELECT p.create,p.read,p.edit,p.delete,p.module from role_permission as rp,permission as p where p.module='booking' and rp.pid = p.pid and rp.rid = (SELECT rid FROM user_role WHERE uid = ?)",
            {
